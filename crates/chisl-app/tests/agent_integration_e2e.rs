...
use chisl_ai_agent::protocol::events::TextEventData;
use chisl_ai_agent::types::{BuildTaskOptions, SendMessageData};
use chisl_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
...
async fn build_app_with_mock_tasks() -> (axum::Router, chisl_app::AppServices, Arc<MockTaskManager>) {
    let db = chisl_db::init_database_memory().await.unwrap();
    let services = chisl_app::AppServices::from_config(db, &chisl_app::AppConfig::default())
        .await
        .unwrap();
    ...
    let router = chisl_app::create_router(&services).await;
