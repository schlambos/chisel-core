...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn chisl_db::IChannelRepository> =
        std::sync::Arc::new(chisl_db::SqliteChannelRepository::new(pool));
    let pairing_svc = chisl_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
