...
async fn insert_message(
    services: &chisl_app::AppServices,
    conv_id: &str,
    msg_id: &str,
    content: &str,
    created_at: i64,
) {
    let repo = chisl_db::SqliteConversationRepository::new(services.database.pool().clone());
    let msg = chisl_db::models::MessageRow {
        id: msg_id.into(),
    ...
    chisl_db::IConversationRepository::insert_message(&repo, &msg)
        .await
        .unwrap();
}

async fn upsert_artifact(services: &chisl_app::AppServices, artifact: chisl_db::ConversationArtifactRow) {
    let repo = chisl_db::SqliteConversationRepository::new(services.database.pool().clone());
    chisl_db::IConversationRepository::upsert_artifact(&repo, &artifact)
        .await
        .unwrap();
}
