...
use chisl_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use chisl_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use chisl_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use chisl_file::FileService;
use chisl_system::VersionCheckService;
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await;
    states.shell.shell_service = std::sync::Arc::new(chisl_shell::ShellService::new(std::sync::Arc::new(
        chisl_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                chisl_ai_agent::types::BuildTaskOptions,
            ) -> futures_util::future::BoxFuture<'static, Result<AgentInstance, chisl_common::AppError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn chisl_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> chisl_common::AgentType {
        chisl_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<chisl_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> chisl_common::TimestampMs {
        chisl_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<chisl_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: chisl_ai_agent::types::SendMessageData,
    ...
    fn kill(&self, _reason: Option<chisl_common::AgentKillReason>) -> Result<(), chisl_common::AppError> {
        Ok(())
    }
}
...
pub async fn setup_and_login(
    ...
    let hash = chisl_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
