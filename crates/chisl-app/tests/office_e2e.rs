...
use chisl_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use chisl_office::{
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use chisl_office::error::OfficeError;
    use chisl_office::types::DocType;
    use chisl_office::{ProcessHandle, ProcessSpawner};
    ...
    impl chisl_realtime::EventBroadcaster for NoopBroadcaster {
        fn broadcast(&self, _msg: chisl_api_types::WebSocketMessage<serde_json::Value>) {}
    }

    let spawner: Arc<dyn ProcessSpawner> = Arc::new(NoopSpawner);
    let bc: Arc<dyn chisl_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
