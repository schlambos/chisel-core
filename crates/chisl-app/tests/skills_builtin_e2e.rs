...
//! `chisl_app::create_router_with_states` against an in-memory DB.

mod common;

use std::sync::Arc;

use chisl_app::{ModuleStates, build_module_states, create_router_with_states};
...
async fn fixture_embedded() -> Fixture {
    ...
    chisl_extension::materialize_if_needed(&data_dir, chisl_extension::builtin_skills_corpus(), "test-fixture")
        .await
