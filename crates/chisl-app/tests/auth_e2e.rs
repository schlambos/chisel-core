...
//! auth routes) via `chisl_app::create_router`, covering test-plan items
...
use chisl_app::{AppConfig, AppServices};
...
async fn t12_3_session_cookie_attributes() {
    ...
    let hash = chisl_auth::hash_password("StrongP@ss1").unwrap();
...
async fn t14_2_setup_then_login() {
    ...
    let hash = chisl_auth::hash_password("Admin@Pass1").unwrap();
    services
        .user_repo
        .set_system_user_credentials("admin", &hash)
        .await
...
async fn full_auth_flow_e2e() {
    ...
    // 2. Setup user
    let hash = chisl_auth::hash_password("Initial@Pass1").unwrap();
    services
        .user_repo
        .set_system_user_credentials("admin", &hash)
        .await
