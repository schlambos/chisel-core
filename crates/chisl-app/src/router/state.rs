...
use chisl_assistant::{AssistantRouterState, AssistantService, BuiltinAssistantRegistry};
use chisl_auth::extract_token_from_ws_headers;
use chisl_channel::ChannelRouterState;
use chisl_conversation::{ConversationRouterState, ConversationService};
use chisl_cron::{CronEventEmitter, CronRouterState};
use chisl_db::{
...
use chisl_file::{FileRouterState, FileService, FileWatchService, SnapshotService};
use chisl_lsp::LspRouterState;
use chisl_mcp::{
...
use chisl_realtime::{EventBroadcaster, NoopMessageRouter, WsHandlerState};
use chisl_shell::ShellRouterState;
use chisl_system::{
...
use chisl_team::{TeamRouterState, TeamSessionService};
use chisl_terminal::{TerminalRouterState, TerminalService};
...
pub struct ChannelOrchestratorComponents {
    pub orchestrator: chisl_channel::orchestrator::ChannelOrchestrator,
    pub message_rx: tokio::sync::mpsc::Receiver<chisl_channel::types::UnifiedIncomingMessage>,
    pub confirm_rx: tokio::sync::mpsc::Receiver<(String, String)>,
    pub manager: Arc<chisl_channel::manager::ChannelManager>,
    pub plugin_factory: Arc<chisl_channel::manager::PluginFactory>,
}
...
pub fn build_conversation_state(
    ...
    cron_service: Option<Arc<chisl_cron::service::CronService>>,
    ...
    let skill_resolver = Arc::new(chisl_conversation::skill_resolver::ExtensionSkillResolver::new(
...
pub fn build_remote_agent_state(services: &AppServices) -> RemoteAgentRouterState {
    ...
    let notifier: Arc<dyn Fn(&str, serde_json::Value) + Send + Sync> = Arc::new(move |name, payload| {
        broadcaster_for_notifier.broadcast(chisl_api_types::WebSocketMessage::new(name, payload));
    });
    chisl_ai_agent::manager::remote::plugin::ui_push::set_ui_notifier(notifier);
...
pub fn build_file_state(services: &AppServices) -> FileRouterState {
    ...
    let browse_roots = chisl_file::browse::default_browse_roots();
    ...
    chisl_ai_agent::manager::remote::edit_inverse::set_pool(services.database.pool().clone());
    ...
    if !tool_snapshot_disabled {
        let tool_repo = Arc::new(chisl_db::SqliteOpencodeToolSnapshotRepository::new(
            services.database.pool().clone(),
        ));
        chisl_ai_agent::manager::remote::local_fs_mcp::snapshot_deps::set(
            chisl_ai_agent::manager::remote::local_fs_mcp::snapshot_deps::SnapshotDeps {
                snapshot_service: snapshot_service.clone(),
                tool_snapshot_repo: tool_repo.clone(),
            },
        );
        chisl_conversation::snapshot_deps::set(chisl_conversation::snapshot_deps::SnapshotDeps {
            snapshot_service: snapshot_service.clone(),
...
pub fn build_mcp_state(services: &AppServices) -> McpRouterState {
    let pool = services.database.pool().clone();
    let repo: Arc<dyn chisl_db::IMcpServerRepository> = Arc::new(chisl_db::SqliteMcpServerRepository::new(pool));
    ...
    let oauth_token_repo: Arc<dyn chisl_db::IOAuthTokenRepository> = Arc::new(
        chisl_db::SqliteOAuthTokenRepository::new(services.database.pool().clone()),
    );
    let http_client = reqwest::Client::new();
    ...
    McpRouterState {
        ...
        oauth_service: chisl_mcp::McpOAuthService::new(oauth_token_repo, http_client),
    }
}
...
pub async fn build_channel_state(
    ...
    let pool = services.database.pool().clone();
    let repo: Arc<dyn chisl_db::IChannelRepository> = Arc::new(chisl_db::SqliteChannelRepository::new(pool));
    let encryption_key = derive_encryption_key(&services.jwt_secret_raw);

    let (message_tx, message_rx) = tokio::sync::mpsc::channel(256);
    let (confirm_tx, confirm_rx) = tokio::sync::mpsc::channel(256);

    let manager = Arc::new(chisl_channel::manager::ChannelManager::new(
        repo.clone(),
        services.event_bus.clone(),
        encryption_key,
        message_tx,
    ...
    let pairing_service = Arc::new(chisl_channel::pairing::PairingService::new(
        repo.clone(),
        services.event_bus.clone(),
    ));

    let session_manager = Arc::new(chisl_channel::session::SessionManager::new(repo.clone()));

    let plugin_factory: Arc<chisl_channel::manager::PluginFactory> =
        Arc::new(Box::new(chisl_channel::plugins::create_plugin));

    // Build channel settings service for per-plugin agent/model configuration
    let pref_pool = services.database.pool().clone();
    let pref_repo: Arc<dyn chisl_db::IClientPreferenceRepository> =
        Arc::new(SqliteClientPreferenceRepository::new(pref_pool));
    let channel_settings = Arc::new(chisl_channel::channel_settings::ChannelSettingsService::new(pref_repo));

    // Build orchestrator dependencies
    let action_executor = Arc::new(chisl_channel::action::ActionExecutor::new(
    ...
    let conv_repo: Arc<dyn chisl_db::IConversationRepository> = Arc::new(
        chisl_db::SqliteConversationRepository::new(services.database.pool().clone()),
    );
    let skill_resolver = Arc::new(chisl_conversation::skill_resolver::ExtensionSkillResolver::new(
        services.skill_paths.clone(),
    ));
    let agent_metadata_repo: Arc<dyn chisl_db::IAgentMetadataRepository> = Arc::new(
        chisl_db::SqliteAgentMetadataRepository::new(services.database.pool().clone()),
    );
    let acp_session_repo: Arc<dyn chisl_db::IAcpSessionRepository> = Arc::new(
        chisl_db::SqliteAcpSessionRepository::new(services.database.pool().clone()),
    );
    ...
    let message_service = Arc::new(chisl_channel::message_service::ChannelMessageService::new(
        conversation_svc,
        services.worker_task_manager.clone(),
        Arc::clone(&channel_settings),
        owner_user_id,
    ...
    let orchestrator = chisl_channel::orchestrator::ChannelOrchestrator::new(
        action_executor,
        message_service,
        Arc::clone(&session_manager),
        manager.clone() as Arc<dyn chisl_channel::stream_relay::ChannelSender>,
    );
...
pub fn build_team_state(
    ...
    guide_mcp_config: Option<chisl_api_types::GuideMcpConfig>,
) -> TeamRouterState {
    let pool = services.database.pool().clone();
    let team_repo: Arc<dyn chisl_db::ITeamRepository> = Arc::new(chisl_db::SqliteTeamRepository::new(pool.clone()));
    let conv_repo: Arc<dyn chisl_db::IConversationRepository> =
...
pub fn build_cron_state(services: &AppServices) -> CronRouterState {
    let pool = services.database.pool().clone();
    let cron_repo: Arc<dyn chisl_db::ICronRepository> = Arc::new(chisl_db::SqliteCronRepository::new(pool.clone()));
    ...
    let busy_guard = Arc::new(chisl_cron::busy_guard::CronBusyGuard::new());
    let executor = Arc::new(chisl_cron::executor::JobExecutor::new(
        services.worker_task_manager.clone(),
        conv_repo,
    ...
    let scheduler = Arc::new(chisl_cron::scheduler::CronScheduler::new(Arc::new(
        move |job_id: String| {
    ...
    let cron_service = Arc::new(chisl_cron::service::CronService::new(
        cron_repo,
        scheduler,
        executor,
        emitter,
...
pub fn build_office_state(services: &AppServices) -> OfficeRouterState {
    ...
    let spawner: Arc<dyn chisl_office::ProcessSpawner> = Arc::new(chisl_office::DefaultProcessSpawner);
...
pub fn build_shell_state(services: &AppServices) -> ShellRouterState {
    ...
    ShellRouterState {
        shell_service: Arc::new(chisl_shell::ShellService::new(Arc::new(
            chisl_shell::DefaultSystemOpener,
        ))),
        stt_service: Arc::new(chisl_shell::SttService::new(reqwest::Client::new())),
        tts_service: Arc::new(chisl_shell::TtsService::new(reqwest::Client::new())),
        client_pref_service,
    }
}
...
#[derive(Default)]
struct CronServiceTickRef(std::sync::Mutex<Option<Arc<chisl_cron::service::CronService>>>);
...
pub async fn build_extension_states(
    ...
    let skill_paths = chisl_extension::resolve_skill_paths(&app_resource_dir, &skill_data_dir);
...
///
/// Pulls the process-global [`chisl_ai_agent::manager::local_opencode::LocalOpenCodeManager`]
...
pub fn build_local_opencode_state(services: &AppServices) -> LocalOpenCodeRouterState {
    let manager = chisl_ai_agent::manager::local_opencode::global();
...
mod tests {
    ...
    use crate::AppConfig;
    use chisl_extension::{ExtensionSource, ScanPath};
