...
use chisl_ai_agent::{agent_routes, local_opencode_routes, remote_agent_routes};
use chisl_assets::{AssetRouterState, asset_routes};
use chisl_assistant::assistant_routes;
use chisl_auth::{
    AuthRouterState, AuthState, auth_middleware, auth_routes, csrf_middleware, security_headers_middleware,
};
use chisl_channel::channel_routes;
use chisl_conversation::{conversation_ops_routes, conversation_routes};
use chisl_cron::cron_routes;
use chisl_extension::{extension_routes, hub_routes, skill_routes};
use chisl_file::file_routes;
use chisl_lsp::{lsp_routes, lsp_ws_routes};
use chisl_mcp::mcp_routes;

use chisl_office::{office_proxy_routes, office_routes};
use chisl_realtime::{WsHandlerState, ws_upgrade_handler};
use chisl_shell::shell_routes;
use chisl_system::{connection_test_routes, system_routes};
use chisl_team::team_routes;
use chisl_terminal::{terminal_routes, terminal_ws_routes};

use crate::config::derive_encryption_key;
use crate::remote_session_sync::RemoteSessionSyncService;
use crate::services::AppServices;
use chisl_ai_agent::RemoteAgentService;
...
pub fn create_router_with_all_state(services: &AppServices, states: ModuleStates, ws_state: WsHandlerState) -> Router {
    ...
    let router = router.layer(DefaultBodyLimit::max(chisl_common::constants::BODY_LIMIT));
