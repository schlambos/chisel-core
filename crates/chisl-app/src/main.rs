...
fn main() -> Result<ExitCode> {
    ...
    // parse a uniform CLI, but bypass `chisl_runtime::init` (which would
    ...
    // detection path exactly. It must hit the same `chisl_runtime::init`
    ...
    if needs_runtime {
        chisl_runtime::init(&cli.data_dir);
    }
    ...
    let merged_path = unsafe { chisl_runtime::enhance_process_path() };
