...
use chisl_auth::{CookieConfig, JwtService, QrTokenStore, resolve_jwt_secret};
...
use chisl_lsp::LspService;
use chisl_realtime::{BroadcastEventBus, WebSocketManager};
use chisl_team::GuideMcpServer;
...
pub struct AppServices {
    ...
    pub skill_paths: Arc<chisl_extension::SkillPaths>,
...
impl AppServices {
    ...
    pub(crate) async fn inject_guide_service(&self, service: std::sync::Weak<chisl_team::TeamSessionService>) {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        ...
        let skill_paths = Arc::new(chisl_extension::resolve_skill_paths(&app_resource_dir, &data_dir));
        ...
        let backend_binary_path =
            Arc::new(std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("chislcore")));
