//! `chislcore mcp-bridge` subcommand: stdio ↔ TCP bridge for the team MCP server.
//!
//! Spawned by the ACP agent CLI as an MCP server with command `chislcore mcp-bridge`.
...
//! `127.0.0.1:<TEAM_MCP_PORT>` (reusing `chisl_team::mcp::protocol`).
//!
...
use chisl_api_types::TeamMcpStdioConfig;
...
/// Entry point for `chislcore mcp-bridge`. Returns an [`ExitCode`] so the
