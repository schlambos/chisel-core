//! `chislcore doctor` subcommand: agent CLI detection self-check.
//!
...
//! `chisl_runtime::init` + `enhance_process_path` for `Doctor` as it
//! does for the server, so the bundled `bun` resolves through the
//! same cache the server uses.
//!
//! Writes to stdout (not the rolling chislcore.log) — the user
...
use chisl_ai_agent::{AgentRegistry, UnavailableReason};
use chisl_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database, maybe_copy_legacy_database};
...
fn print_snapshot(snapshot: &[(chisl_api_types::AgentMetadata, Option<UnavailableReason>)]) {
