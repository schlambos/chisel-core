...
use chisl_common::constants::UPLOAD_MAX_SIZE;
...
pub fn file_routes(state: FileRouterState) -> Router {
    ...
    // We first disable the global `DefaultBodyLimit` that `chisl-app`
...
fn to_zip_entry(e: chisl_api_types::ZipFileEntry) -> ZipEntry {
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let mode = match info.mode {
        SnapshotMode::GitRepo => chisl_api_types::SnapshotMode::GitRepo,
        SnapshotMode::Snapshot => chisl_api_types::SnapshotMode::Snapshot,
    };
...
mod tests {
    ...
    fn zip_entry_conversion_text() {
        let e = chisl_api_types::ZipFileEntry {
            name: "a.txt".into(),
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, chisl_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, chisl_api_types::SnapshotMode::Snapshot);
