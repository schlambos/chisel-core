...
use chisl_ai_agent::AgentRegistry;
use chisl_ai_agent::agent_task::AgentInstance;
use chisl_ai_agent::types::BuildTaskOptions;
use chisl_api_types::{
    CreateCronJobRequest, CronScheduleDto, ListCronJobsQuery, SaveCronSkillRequest, UpdateCronJobRequest,
    WebSocketMessage,
};
use chisl_common::{PaginatedResult, TimestampMs, now_ms};
use chisl_conversation::ConversationService;
use chisl_conversation::response_middleware::{CronCreateParams, CronUpdateParams};
use chisl_db::{
...
use chisl_cron::busy_guard::CronBusyGuard;
use chisl_cron::events::CronEventEmitter;
use chisl_cron::executor::JobExecutor;
use chisl_cron::scheduler::CronScheduler;
use chisl_cron::service::CronService;
use chisl_cron::types::JobStatus;
...
impl chisl_ai_agent::task_manager::IWorkerTaskManager for StubTaskManager {
    fn get_task(&self, _: &str) -> Option<AgentInstance> {
        None
    }
    async fn get_or_build_task(&self, _: &str, _: BuildTaskOptions) -> Result<AgentInstance, chisl_common::AppError> {
...
struct StubConvRepo {
    ...
    artifacts: Mutex<Vec<chisl_db::ConversationArtifactRow>>,
    rows: Mutex<HashMap<String, chisl_db::models::ConversationRow>>,
}
...
impl StubConvRepo {
    ...
    fn upsert_artifact_row(&self, artifact: chisl_db::ConversationArtifactRow) {
    ...
    fn artifacts(&self) -> Vec<chisl_db::ConversationArtifactRow> {
...
impl IConversationRepository for StubConvRepo {
    async fn get(&self, id: &str) -> Result<Option<chisl_db::models::ConversationRow>, chisl_db::DbError> {
        ...
        let row = if id == "conv_mode" {
            chisl_db::models::ConversationRow {
                id: id.into(),
                user_id: "u1".into(),
    ...
    async fn create(&self, row: &chisl_db::models::ConversationRow) -> Result<(), chisl_db::DbError> {
        self.rows.lock().unwrap().insert(row.id.clone(), row.clone());
        Ok(())
    }
    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), chisl_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let row = rows
            .entry(id.to_owned())
            .or_insert_with(|| chisl_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: "u1".into(),
                name: "stub".into(),
                r#type: "default".into(),
    ...
    async fn list_artifacts(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<chisl_db::ConversationArtifactRow>, chisl_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<chisl_db::ConversationArtifactRow>, chisl_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn upsert_artifact(
        &self,
        artifact: &chisl_db::ConversationArtifactRow,
    ) -> Result<chisl_db::ConversationArtifactRow, chisl_db::DbError> {
...
async fn setup_with_conv_repo() -> (
    ...
    let task_manager: Arc<dyn chisl_ai_agent::task_manager::IWorkerTaskManager> = Arc::new(StubTaskManager);
...
async fn cj5_get_nonexistent_job() {
    ...
    assert!(matches!(err, chisl_cron::error::CronError::JobNotFound(_)));
}
...
async fn cj9_update_schedule_type() {
    ...
    assert!(matches!(
        updated.schedule,
        chisl_cron::types::CronSchedule::Cron { .. }
    ));
...
async fn cj12_delete_nonexistent() {
    ...
    assert!(matches!(
        err,
        chisl_cron::error::CronError::Database(chisl_db::DbError::NotFound(_))
    ));
}
...
async fn sk1_1_save_skill_marks_related_skill_suggest_artifacts_saved() {
    ...
    conv_repo.upsert_artifact_row(chisl_db::ConversationArtifactRow {
...
async fn sk4_save_empty_skill() {
    ...
    let err = svc
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, chisl_cron::error::CronError::InvalidSkillContent(_)));
}
...
async fn sc5_invalid_cron_expression() {
    ...
    assert!(matches!(err, chisl_cron::error::CronError::InvalidCronExpression(_)));
}
...
async fn sc7_every_zero_interval() {
    ...
    let err = svc.add_job(req).await.unwrap_err();
    assert!(matches!(err, chisl_cron::error::CronError::InvalidSchedule(_)));
}
...
async fn icron_service_create_job() {
    ...
    use chisl_conversation::response_middleware::ICronService;
...
async fn sr1_system_resume_missed_job() {
    ...
    let params = chisl_db::UpdateCronJobParams {
...
async fn cd3_on_conversation_delete_trait() {
    use chisl_common::OnConversationDelete;
