...
use chisl_api_types::{ConversationArtifactResponse, WebSocketMessage};
use chisl_common::generate_id;
use chisl_db::ConversationArtifactRow;
...
mod tests {
    ...
    fn builds_skill_suggest_response() {
        ...
        assert_eq!(response.kind, chisl_api_types::ConversationArtifactKind::SkillSuggest);
        assert_eq!(response.status, chisl_api_types::ConversationArtifactStatus::Pending);
    ...
    fn builds_cron_trigger_payload() {
        ...
        let response = artifact_response_from_row(&row).unwrap();
        assert_eq!(response.kind, chisl_api_types::ConversationArtifactKind::CronTrigger);
