...
use chisl_ai_agent::task_manager::IWorkerTaskManager;
use chisl_ai_agent::types::{BuildTaskOptions, SendMessageData};
use chisl_ai_agent::{AgentRegistry, AgentStreamEvent};
use chisl_api_types::{CreateConversationRequest, SendMessageRequest};
use chisl_common::{AgentType, ProviderWithModel, now_ms};
use chisl_conversation::ConversationService;
use chisl_db::models::MessageRow;
use chisl_db::{ConversationRowUpdate, IConversationRepository};
use chisl_realtime::EventBroadcaster;
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<chisl_db::models::ConversationRow>, CronError> {
    ...
    pub async fn insert_tips_message(
        ...
        let row = MessageRow {
            ...
            hidden: false,
            created_at: chisl_common::now_ms(),
        };
...
impl JobExecutor {
    ...
    async fn execute_inner(
        ...
        // `chisl_conversation::task_options::provider_model_from_conversation_row`.
        ...
        let model = match self.get_conversation_row(conversation_id).await {
            Ok(Some(row)) => chisl_conversation::task_options::provider_model_from_conversation_row(&row),
            Ok(None) => chisl_conversation::task_options::empty_provider_model(),
            Err(e) => {
                error!(
                    job_id = %job.id,
                    conversation_id,
    ...
    fn spawn_skill_suggest_flow(
        &self,
        agent: chisl_ai_agent::AgentInstance,
...
mod tests {
    ...
    use chisl_ai_agent::agent_task::{AgentInstance, IAgentTask, IMockAgent};
    use chisl_ai_agent::protocol::events::FinishEventData;
    use chisl_api_types::{AgentModeResponse, WebSocketMessage};
    use chisl_common::{AgentKillReason, ConversationStatus, PaginatedResult, TimestampMs};
    use chisl_db::{
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        let db = chisl_db::init_database_memory().await.unwrap();
        let repo = Arc::new(chisl_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    fn make_executor_for_busy_tests(guard: Arc<CronBusyGuard>) -> JobExecutor {
        ...
        impl IWorkerTaskManager for StubTaskManager {
            ...
            async fn get_or_build_task(
                ...
                _: &str,
                _: BuildTaskOptions,
            ) -> Result<AgentInstance, chisl_common::AppError> {
                Err(chisl_common::AppError::Internal("stub".into()))
            }
            fn kill(&self, _: &str, _: Option<chisl_common::AgentKillReason>) -> Result<(), chisl_common::AppError> {
                Ok(())
            }
            fn kill_and_wait(
                &self,
                _: &str,
                _: Option<chisl_common::AgentKillReason>,
            ...
            fn collect_idle(&self, _: chisl_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl IConversationRepository for StubConvRepo {
            async fn get(&self, _id: &str) -> Result<Option<chisl_db::models::ConversationRow>, chisl_db::DbError> {
                Ok(None)
            }
            async fn create(&self, _row: &chisl_db::models::ConversationRow) -> Result<(), chisl_db::DbError> {
                Ok(())
            }
            async fn update(&self, _id: &str, _updates: &ConversationRowUpdate) -> Result<(), chisl_db::DbError> {
                Ok(())
            }
            async fn delete(&self, _id: &str) -> Result<(), chisl_db::DbError> {
                Ok(())
            }
            ...
            ) -> Result<PaginatedResult<chisl_db::models::ConversationRow>, chisl_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
            ...
            async fn find_by_source_and_chat(
                ...
                _chat_id: &str,
                _agent_type: &str,
            ) -> Result<Option<chisl_db::models::ConversationRow>, chisl_db::DbError> {
                Ok(None)
            }
            async fn list_by_cron_job(
                ...
                _user_id: &str,
                _cron_job_id: &str,
            ) -> Result<Vec<chisl_db::models::ConversationRow>, chisl_db::DbError> {
                Ok(vec![])
            }
            ...
            async fn get_messages(
                ...
                _page_size: u32,
                _order: SortOrder,
            ) -> Result<PaginatedResult<chisl_db::models::MessageRow>, chisl_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
            ...
            async fn insert_message(&self, _message: &chisl_db::models::MessageRow) -> Result<(), chisl_db::DbError> {
                Ok(())
            }
            async fn update_message(&self, _id: &str, _updates: &MessageRowUpdate) -> Result<(), chisl_db::DbError> {
                Ok(())
            }
            async fn delete_messages_by_conversation(&self, _conv_id: &str) -> Result<(), chisl_db::DbError> {
                Ok(())
            }
            async fn get_message_by_msg_id(
                ...
                _msg_id: &str,
                _msg_type: &str,
            ) -> Result<Option<chisl_db::models::MessageRow>, chisl_db::DbError> {
                Ok(None)
            }
            async fn search_messages(
                ...
                _page: u32,
                _page_size: u32,
            ) -> Result<PaginatedResult<MessageSearchRow>, chisl_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
        ...
        impl chisl_realtime::EventBroadcaster for StubBroadcaster {
            fn broadcast(&self, _: WebSocketMessage<serde_json::Value>) {}
        }

        struct StubSkillResolver;
        #[async_trait::async_trait]
        impl chisl_conversation::skill_resolver::SkillResolver for StubSkillResolver {
            ...
            async fn resolve_skills(
                &self,
                _names: &[String],
            ) -> Vec<chisl_conversation::skill_resolver::ResolvedAgentSkill> {
                Vec::new()
            }
            ...
            async fn link_workspace_skills(
                ...
                _rel_dirs: &[&str],
                _skills: &[chisl_conversation::skill_resolver::ResolvedAgentSkill],
            ) -> usize {
                0
            }
        }

        let stub_broadcaster: Arc<dyn chisl_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
        let stub_repo: Arc<dyn IConversationRepository> = Arc::new(StubConvRepo);
        let agent_metadata_repo: Arc<dyn chisl_db::IAgentMetadataRepository> = Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn chisl_db::IAcpSessionRepository> = Arc::new(StubAcpSessionRepo);
    ...
    impl IAgentTask for RecordingAgent {
        ...
        async fn send_message(&self, data: SendMessageData) -> Result<(), chisl_common::AppError> {
            self.send_calls.fetch_add(1, Ordering::Relaxed);
            self.sent_messages.write().await.push(data);
            Ok(())
        }

        async fn cancel(&self) -> Result<(), chisl_common::AppError> {
            Ok(())
        }

        fn kill(&self, _reason: Option<AgentKillReason>) -> Result<(), chisl_common::AppError> {
            Ok(())
        }
    }
    ...
    impl IMockAgent for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, chisl_common::AppError> {
        ...
        async fn set_mode(&self, mode: &str) -> Result<(), chisl_common::AppError> {
    ...
    impl IConversationRepository for ExistingConversationRepo {
        async fn get(&self, id: &str) -> Result<Option<chisl_db::models::ConversationRow>, chisl_db::DbError> {
            Ok(Some(chisl_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: "cron".into(),
    ...
    struct MissingWorkspaceConversationRepo {
        row: chisl_db::models::ConversationRow,
        updates: Mutex<Vec<ConversationRowUpdate>>,
        inserted_messages: Mutex<Vec<chisl_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<chisl_db::models::MessageRow> {
    ...
    impl chisl_realtime::EventBroadcaster for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        async fn update(&self, _id: &str, updates: &ConversationRowUpdate) -> Result<(), chisl_db::DbError> {
        ...
        async fn insert_message(&self, message: &chisl_db::models::MessageRow) -> Result<(), chisl_db::DbError> {
        ...
        ) -> Result<ConversationArtifactRow, chisl_db::DbError> {
    ...
    fn make_executor_with_task_manager_and_repo(
        ...
        let broadcaster: Arc<dyn chisl_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
    ...
    fn make_executor_with_task_manager_repo_and_broadcaster(
        task_manager: Arc<dyn IWorkerTaskManager>,
        repo: Arc<dyn IConversationRepository>,
        broadcaster: Arc<dyn chisl_realtime::EventBroadcaster>,
    ) -> JobExecutor {
        struct StubSkillResolver;
    ...
    #[async_trait::async_trait]
    impl chisl_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<chisl_db::models::AcpSessionRow>, chisl_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &chisl_db::CreateAcpSessionParams<'_>,
        ) -> Result<chisl_db::models::AcpSessionRow, chisl_db::DbError> {
            Err(chisl_db::DbError::Init("stub".into()))
        }
        async fn update_session_id(
            &self,
            _conversation_id: &str,
            _session_id: &str,
        ) -> Result<bool, chisl_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _conversation_id: &str) -> Result<bool, chisl_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<chisl_db::PersistedSessionState>, chisl_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state(
            &self,
            _conversation_id: &str,
            _params: &chisl_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, chisl_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl chisl_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        async fn list_all(&self) -> Result<Vec<chisl_db::models::AgentMetadataRow>, chisl_db::DbError> {
            Ok(Vec::new())
        }
        async fn get(&self, _id: &str) -> Result<Option<chisl_db::models::AgentMetadataRow>, chisl_db::DbError> {
            Ok(None)
        }
        async fn find_by_source_and_name(
            ...
            _agent_source: &str,
            _name: &str,
        ) -> Result<Option<chisl_db::models::AgentMetadataRow>, chisl_db::DbError> {
            Ok(None)
        }
        ...
        async fn upsert(
            &self,
            _params: &chisl_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<chisl_db::models::AgentMetadataRow, chisl_db::DbError> {
            Err(chisl_db::DbError::Init("stub".into()))
        }
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &chisl_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<chisl_db::models::AgentMetadataRow>, chisl_db::DbError> {
            Ok(None)
        }
        async fn set_enabled(&self, _id: &str, _enabled: bool) -> Result<bool, chisl_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _id: &str) -> Result<bool, chisl_db::DbError> {
            Ok(false)
        }
    }
}
