use chisl_db::models::AgentMetadataRow;
...
pub fn is_whitelisted_backend(backend: &str) -> bool {
    chisl_common::constants::TEAM_CAPABLE_BACKENDS.contains(&backend)
}
        ...
        "team_spawn_agent" => {
            ...
            if !is_whitelisted_backend(&backend) {
                return Err(format!(
                    ...
                    backend,
                    chisl_common::constants::TEAM_CAPABLE_BACKENDS.join(", ")
                ));
            }
...
pub fn build_list_models_from_rows(
    ...
    use chisl_api_types::BehaviorPolicy;
    use chisl_common::constants::is_team_capable;
