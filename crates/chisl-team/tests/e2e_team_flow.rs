...
use chisl_ai_agent::protocol::events::{AgentStreamEvent, FinishEventData};
use chisl_ai_agent::shared_kernel::approval_key;
use chisl_ai_agent::types::{BuildTaskOptions, SendMessageData};
use chisl_api_types::AgentModeResponse;
use chisl_api_types::WebSocketMessage;
use chisl_common::{AgentKillReason, AgentType, AppError, Confirmation, ConversationStatus, TimestampMs, now_ms};
use chisl_db::ITeamRepository;
use chisl_realtime::EventBroadcaster;
use chisl_team::mcp::protocol::{read_frame, write_frame};
use chisl_team::service::TeamSessionService;
use chisl_team::{TeamAgent, TeamSession, TeammateRole};
...
#[async_trait]
impl chisl_ai_agent::IWorkerTaskManager for StubTaskManager {
...
fn backend_path() -> Arc<PathBuf> {
    Arc::new(PathBuf::from("/tmp/chislcore-e2e-test"))
}
...
async fn setup_session() -> (
    ...
    let task_manager_dyn: Arc<dyn chisl_ai_agent::IWorkerTaskManager> = task_manager.clone();

    let team = chisl_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
        agents: two_agents(),
...
async fn s3a_on_agent_finish_writes_idle_notification_to_lead() {
    ...
    session
        .scheduler()
        .set_status("worker-1", chisl_team::TeammateStatus::Working)
        .await
        .unwrap();
    ...
    assert_eq!(
        worker_status,
        chisl_team::TeammateStatus::Idle,
...
async fn s3b_lead_finish_does_not_write_idle_notification() {
    ...
    session
        .scheduler()
        .set_status("lead-1", chisl_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3c_finish_triggers_lead_wake_with_idle_notification() {
    ...
    session
        ...
        .write(
            ...
            "worker-1",
            "lead-1",
            chisl_team::MailboxMessageType::Message,
            "do the work",
            None,
        )
        .await
...
async fn s10_error_finish_sets_agent_status_to_error() {
    ...
    assert_eq!(
        status,
        chisl_team::TeammateStatus::Error,
