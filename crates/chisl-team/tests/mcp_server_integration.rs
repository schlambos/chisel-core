...
use chisl_team::{Mailbox, TaskBoard, TeamAgent, TeamMcpServer, TeammateManager, TeammateRole};
...
async fn sb1_bridge_config_generation() {
    use chisl_team::{TeamMcpStdioConfig, TeamMcpStdioServerSpec};
    ...
    let config = TeamMcpStdioConfig {
        ...
        slot_id: "lead-1".into(),
        binary_path: "/bin/chislcore".into(),
    };

    let spec = TeamMcpStdioServerSpec::from_config("/bin/chislcore", &config);
...
async fn mcp_status_tcp_ready_is_broadcast_on_successful_bind() {
    use chisl_api_types::{TeamMcpPhase, TeamMcpStatusPayload};
