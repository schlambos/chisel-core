...
use chisl_ai_agent::task_manager::AgentFactory;
use chisl_ai_agent::types::BuildTaskOptions;
use chisl_ai_agent::{IWorkerTaskManager, WorkerTaskManagerImpl};
use chisl_api_types::{AddAgentRequest, CreateTeamRequest, TeamAgentInput, WebSocketMessage};
use chisl_common::{AgentKillReason, AppError, PaginatedResult};
use chisl_db::models::{
...
use chisl_team::TeamSessionService;
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<chisl_db::models::TeamRow>>,
}
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &chisl_db::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams(&self) -> Result<Vec<chisl_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn get_team(&self, id: &str) -> Result<Option<chisl_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().iter().find(|t| t.id == id).cloned())
    }
    async fn update_team(&self, id: &str, params: &chisl_db::UpdateTeamParams) -> Result<(), DbError> {
        ...
        team.updated_at = chisl_common::now_ms();
        Ok(())
    }
    ...
    async fn write_message(&self, row: &chisl_db::models::MailboxMessageRow) -> Result<(), DbError> {
    ...
    async fn read_unread_and_mark(
        ...
        team_id: &str,
        to_agent_id: &str,
    ) -> Result<Vec<chisl_db::models::MailboxMessageRow>, DbError> {
    ...
    async fn create_task(&self, row: &chisl_db::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<chisl_db::models::TeamTaskRow>, DbError> {
        self.inner.find_task_by_id(team_id, task_id).await
    }
    async fn update_task(&self, task_id: &str, params: &chisl_db::UpdateTaskParams) -> Result<(), DbError> {
        self.inner.update_task(task_id, params).await
    }
    async fn list_tasks(&self, team_id: &str) -> Result<Vec<chisl_db::models::TeamTaskRow>, DbError> {
...
impl chisl_conversation::skill_resolver::SkillResolver for StubSkillResolver {
    ...
    async fn resolve_skills(&self, _names: &[String]) -> Vec<chisl_conversation::skill_resolver::ResolvedAgentSkill> {
        Vec::new()
    }
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<chisl_ai_agent::AgentInstance> {
    ...
    ) -> Result<chisl_ai_agent::AgentInstance, AppError> {
    ...
    fn collect_idle(&self, idle_threshold_ms: chisl_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    use chisl_ai_agent::agent_task::{IAgentTask, IMockAgent};
    use chisl_ai_agent::protocol::events::AgentStreamEvent;
    use chisl_ai_agent::types::SendMessageData;
    use chisl_common::{AgentKillReason, AgentType, AppError, ConversationStatus, TimestampMs};
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(chisl_ai_agent::AgentInstance::Mock(Arc::new(
...
impl IProviderRepository for EmptyProviderRepo {
    async fn list(&self) -> Result<Vec<chisl_db::models::Provider>, DbError> {
        Ok(vec![])
    }
    async fn find_by_id(&self, _id: &str) -> Result<Option<chisl_db::models::Provider>, DbError> {
        Ok(None)
    }
    async fn create(
        &self,
        _params: chisl_db::CreateProviderParams<'_>,
    ) -> Result<chisl_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
    async fn update(
        &self,
        _id: &str,
        _params: chisl_db::UpdateProviderParams<'_>,
    ) -> Result<chisl_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
...
fn setup_with_factory_and_metadata(
    ...
    let backend_binary_path = Arc::new(std::path::PathBuf::from("/tmp/chislcore-test"));
...
async fn d31b2_ensure_session_broadcasts_load_failed_for_missing_team() {
    ...
    assert!(matches!(err, chisl_team::TeamError::TeamNotFound(_)));
