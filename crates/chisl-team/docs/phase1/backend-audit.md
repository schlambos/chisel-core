...
>
> - [`docs/teams/phase1/chislui-audit.md`](./chislui-audit.md) — **AionUi 侧权威事实来源**（搭档 chislui-audit 交付，锚定 AionUi main @ `ed8a6bcd3`）
...
> **版本说明**：本文档第 3–9 节已基于 chislui-audit.md（AionUi commit `ed8a6bcd3`）对齐；之前凭推断得出的结论已按源码事实修正。所有 "AionUi 有 ✅" 的列都能在 chislui-audit.md 里锚定到行号。
...
**P0 gap 的本质**（对照 chislui-audit.md §7）：chislui-backend 目前停在"AionUi 架构的数据结构层"，agent 运行时、MCP 注入链、Prompt 内容、可靠性机制全部缺失。只有 team 数据面先跑通 MCP 工具注入 + 订阅 ACP Finish 事件回调 `finalize_turn`，用户才可能第一次看到 "lead 在对话里调度 teammate" 的真实行为。
...
| `TeammateStatus` ([types.rs:42-82](../../../crates/chislui-team/src/types.rs)) | `Idle` / `Working` / `Thinking` / `ToolUse` / `Completed` / `Error`；接受 AionUi 别名（pending/active/failed）                                  |
...
> **数据来源**：下表每一行的"AionUi 有 ✅" 均能锚定到 [chislui-audit.md](./chislui-audit.md) 的具体章节（主 = 该文档行号）。本轮对照后，先前凭推断填的列已全部删除，事实不明的行标"未在源码找到"。
>
> GAP 按 AionUi 分类重排：**MCP 注入链 → Scheduler 可靠性 → Prompt 体系 → Agent 生命周期 → 数据层 → 事件层 → 协议层**。

### 3.1 P0（没它 team 跑不起来）

| #   | 能力                                                                                                    | 后端有  | AionUi | GAP                                                                                                                                                                                                                                                                                                                                                                                                        | chislui-audit 锚点                                            |
| --- | ------------------------------------------------------------------------------------------------------- | :-----: | :----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
...
| 9   | Layer-3 Teammate prompt 全内容注入                                                                      |   ⚠️    |   ✅   | `build_teammate_prompt`（[prompts.rs:62-89](../../../crates/chislui-team/src/prompts.rs)）三段概要；AionUi `teammatePrompt.ts:85-97` 显式 "Standing By (300s 超时防护)" + "Shutdown 协议原文"                                                                                                                                                                                                               | §5.1                                                         |
| 10  | 10 个 team 内部 MCP 工具                                                                                | ⚠️ 8/10 |   ✅   | 缺 `team_describe_assistant`、`team_list_models`；`team_members` 输出格式和 `team_task_list` ID 截断（[server.rs:427-443](../../../crates/chislui-team/src/mcp/server.rs)）与 AionUi 对齐但需要验证                                                                                                                                                                                                         | §3.2 Team 内部 10 个工具                                     |
...
| 20  | Leader 不可 shutdown / remove 的硬约束                                                                                                                                           | ⚠️ 部分 |   ✅    | `shutdown_agent` 检查了 Lead-only caller（[scheduler.rs:451-455](../../../crates/chislui-team/src/scheduler.rs)），但目标 agent role 未检查；`remove_agent` 完全不检查 role。AionUi 两处都拒                                                                                                                                                                 | §8 #6                                       |
...
| 22  | `team_spawn_agent` 真正 spawn teammate conversation + agent task                                                                                                                 |   ❌    |   ✅    | scheduler 只 log 不执行（[scheduler.rs:254-266](../../../crates/chislui-team/src/scheduler.rs)）；AionUi 走 `spawnAgent` 闭包 → `addAgent` → 写 `teamMcpStdioConfig` 回 extra → wake 新 agent                                                                                                                                                                | §2.1 MCP spawn、§1.3 getOrStartSession 闭包 |
...
| 24  | `team_rename_agent` 规范化（trim + 去不可见字符 + 小写唯一性校验） + 保留 originalName 供 prompt                                                                                 |   ❌    |   ✅    | 后端直接写新 name（[scheduler.rs:336-346](../../../crates/chislui-team/src/scheduler.rs)）；AionUi 额外做规范化 + 唯一性 + `renamedAgents` map（供 prompt 显示 `[formerly: X]`）                                                                                                                                                                             | §2.1 renameAgent                            |
| 25  | `TeammateStatus` 枚举 `pending` / `idle` / `active` / `completed` / `failed`                                                                                                     |   ⚠️    |   ✅    | 后端是 `Idle` / `Working` / `Thinking` / `ToolUse` / `Completed` / `Error`（[types.rs:42-55](../../../crates/chislui-team/src/types.rs)）。语义差异：AionUi 的 `pending` = 首次 wake 前的 "未启动"；后端没有这个状态。并且接受 AionUi 别名（`pending`→Idle, `active`→Working, `failed`→Error）**已经做了**，但少了 `pending` 触发首次 role prompt 注入的区分 | §2.2 状态机                                 |
...
| 30  | `isTeamCapableBackend(backend, caps)` 白名单 + ACP stdio 能力判断                                                                                                                |   ❌    |   ✅    | 后端硬编码 `["claude", "codex"]`（[tools.rs:167](../../../crates/chislui-team/src/mcp/tools.rs)），仅用于 `team_spawn_agent` 白名单。AionUi 白名单是 `{gemini, claude, codex, aionrs}` + 其他 backend 按 `cachedInitResults.capabilities.mcpCapabilities.stdio===true` 动态判定                                                                              | §3.1 "Team Guide 能力判断"                  |
| 31  | User-scope 过滤（list_teams / get_team / remove_team）                                                                                                                           |   ❌    |   ✅    | [service.rs:127-170](../../../crates/chislui-team/src/service.rs) 未按 user_id 过滤。AionUi `listTeams(userId)` 直接按 user 查                                                                                                                                                                                                                               | §1.1 列 team                                |
...
| 39  | `CreateTeamRequest.agents[].role` 字段尊重                                                                                                                                              |   ❌   |   ✅   | 后端强制 i==0 为 Lead（[service.rs:56-60](../../../crates/chislui-team/src/service.rs)）；AionUi 允许输入指定 |
...
| #   | 能力                                                                                                                    | 后端有 | AionUi | GAP                                                                                                                                                                                                                                                                                                                                                              | 优先级 | chislui-audit 锚点                         |
| --- | ----------------------------------------------------------------------------------------------------------------------- | :----: | :----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | ----------------------------------------- |
| 45  | `send_message` / `send_message_to_agent` 支持附件 `files`                                                               |   ❌   |   ✅   | 后端 [`routes.rs:136-144`](../../../crates/chislui-team/src/routes.rs) 的 `SendTeamMessageRequest` / `SendAgentMessageRequest` 只有 `content` 字段；AionUi 签名 `{teamId, slotId, content, silent?, files?}`                                                                                                                                                      | **P0** | §7.1 "对 team 发话" / "对 agent 发话"     |
...
| 48  | 工具描述原文常量：`TEAM_SPAWN_AGENT_DESCRIPTION` 常量 + `getCreateTeamToolDescription()` 动态生成器                     |   ❌   |   ✅   | 后端 [`tools.rs:30-45`](../../../crates/chislui-team/src/mcp/tools.rs) 是极简自造描述（"Dynamically create a new teammate agent (Lead only)."）；AionUi 对应描述是"3 PRECONDITIONS + STRICT 流程"的长文本，影响 agent 是否会在 spawn 前征求用户同意                                                                                                               | **P0** | §5.3、§7.4                                |
...
| 50  | `formatMessages(messages, agents)` 邮件格式化：`[From <senderName\|User>] <content>\nFiles: <joined>`                   |   ❌   |   ✅   | 后端 [`prompts.rs:98-117`](../../../crates/chislui-team/src/prompts.rs) 的 `build_wake_payload` 用 "- From \`slot_id\` [type]: content" 格式，和 AionUi 不一致；会影响 LLM 对消息来源的解析                                                                                                                                                                       | **P1** | §7.4、§2.1 "消息格式"                     |
| 51  | MCP spawn 路径硬编码 `sessionMode='yolo'` / `workspaceMode='shared'` / `userId='system_default_user'`                   |   ❌   |   ✅   | 后端 `create_team` 接受 userId 参数（[`service.rs:39`](../../../crates/chislui-team/src/service.rs)），但本地开发模式用 `system_default_user` 的约定未写入；`sessionMode` 字段后端 `TeamSessionService` 根本没传给新建的 conversation（[`service.rs:63-74`](../../../crates/chislui-team/src/service.rs) 只传 `teamId`）。AionUi 这三个是 MCP-spawn 建团的硬默认值 | **P1** | §8 #15/#16、§1.2 建团流程                 |
...
| 54  | `TeammateManager.removeAgent` 要清理的内部表                                                                            |   ❌   |   ✅   | 后端 [`scheduler.rs:324-333`](../../../crates/chislui-team/src/scheduler.rs) 只从 slots 移除；AionUi 还要清 `wakeTimeouts / activeWakes / ownedConversationIds / finalizedTurns`（#13-15 的实现先决条件）                                                                                                                                                         | **P1** | §2.1 TeammateManager.removeAgent          |
| 55  | `TeamTask.owner` 要允许 slotId 以外的 sentinel（比如 `null` / empty 表示 unassigned，AionUi 工具输出显示 "unassigned"） |   ✅   |   ✅   | 后端 `TeamTask.owner` 是 `Option<String>`（已支持），**但 `team_task_list` 输出格式**（[`mcp/server.rs:436-437`](../../../crates/chislui-team/src/mcp/server.rs)）要对齐 AionUi 的 `- [<id8>] <subject> (<status>, owner: <X> \| unassigned)` 格式                                                                                                                | **P2** | §3.2 `team_task_list`                     |
...
5. `prompts.rs` 大幅扩写（P0#8/#9）：
   ...
   - **文本必须原样复用** AionUi `leadPrompt.ts` / `teammatePrompt.ts` / `teamGuidePrompt.ts`（chislui-audit §8 #5 强调这一点）；现有的极简中文模板要全部替换
...
| 文件                                         | 改动摘要                                                                                                                                                                                                                              |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `crates/chislui-ai-agent/src/types.rs`        | `AcpBuildExtra` 加 `team_mcp_stdio_config: Option<TeamMcpStdioConfig>`（字段名对齐 AionUi `teamMcpStdioConfig`）                                                                                                                      |
...
| `crates/chislui-team/src/prompts.rs`          | **整个文件重写**：leader/teammate 模板文本原样复用 AionUi `leadPrompt.ts` / `teammatePrompt.ts`（见 [team-prompts.md §1-§4](../team-prompts.md)）；新增动态参数 `availableAgentTypes/availableAssistants/renamedAgents/teamWorkspace` |
...
| -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AionUi 建 team 后，每个 agent 的 ACP `session/new.mcp_servers` 是否带 team MCP？ | **是**。`src/process/agent/acp/index.ts:1605-1656` 用 `loadBuiltinSessionMcpServers()` 把 `extra.teamMcpStdioConfig` 包成 `AcpSessionMcpServer` 注入。见 chislui-audit §2.1 / §3.1。                                                                                                                                                                |
...
6. **Rust 端 stdio bridge 的打包方式**：AionUi 的 bridge 是 `node + scriptPath`；后端如果要重新实现 bridge，是打成独立 `chislui-team-mcp-bridge` 二进制（app 发布时一起分发）还是用子命令 `chislui-backend team-mcp-bridge`。这决定了 `McpServer::Stdio.command` 的值。
