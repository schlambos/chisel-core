...
//! crates (`chisl-ai-agent` deserializing `AcpBuildExtra`, etc.) can reference
...
mod tests {
    ...
    fn deserialization_tolerates_unknown_fields() {
        ...
        let json = r#"{"team_id":"t-1","port":1,"token":"t","slot_id":"s","binary_path":"/usr/bin/chislcore","future_field":42}"#;
        let parsed: TeamMcpStdioConfig = serde_json::from_str(json).unwrap();
