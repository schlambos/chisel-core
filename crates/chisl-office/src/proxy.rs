...
impl From<ProxyError> for chisl_common::AppError {
    fn from(err: ProxyError) -> Self {
        match err {
            ProxyError::PortNotActive(_) => chisl_common::AppError::Forbidden(err.to_string()),
            ProxyError::Timeout => chisl_common::AppError::Timeout(err.to_string()),
            ProxyError::ConnectionFailed(msg) => chisl_common::AppError::BadGateway(msg),
            ProxyError::RequestFailed(msg) => chisl_common::AppError::BadGateway(msg),
        }
    }
}
...
mod tests {
    ...
    fn proxy_error_port_not_active_to_forbidden() {
        let err: chisl_common::AppError = ProxyError::PortNotActive(8080).into();
        assert!(matches!(err, chisl_common::AppError::Forbidden(_)));
    }

    #[test]
    fn proxy_error_timeout_to_timeout() {
        let err: chisl_common::AppError = ProxyError::Timeout.into();
        assert!(matches!(err, chisl_common::AppError::Timeout(_)));
    }

    #[test]
    fn proxy_error_connection_failed_to_bad_gateway() {
        let err: chisl_common::AppError = ProxyError::ConnectionFailed("refused".into()).into();
        assert!(matches!(err, chisl_common::AppError::BadGateway(_)));
    }

    #[test]
    fn proxy_error_request_failed_to_bad_gateway() {
        let err: chisl_common::AppError = ProxyError::RequestFailed("network error".into()).into();
        assert!(matches!(err, chisl_common::AppError::BadGateway(_)));
    }
