...
- 其他 9 个 team\_\* 工具 description：**静态**写在 `teamMcpStdio.ts:83-283` inline。
...
**Team 内部 MCP（team 成员用，带 `from_slot_id`）** 3. `team_send_message(to, message, summary?)` — 含 `*` 广播、`shutdown_approved/rejected` 拦截4. `team_spawn_agent(name, agent_type?, custom_agent_id?, model?)` — leader-only 5. `team_task_create(subject, description?, owner?)` 6. `team_task_update(task_id, status?, owner?)` — completed 自动 `checkUnblocks` 7. `team_task_list` 8. `team_members` 9. `team_rename_agent(agent, new_name)` 10. `team_shutdown_agent(agent)` — leader 不可 shutdown 11. `team_describe_assistant(custom_agent_id, locale?)` 12. `team_list_models(agent_type?)`
...
- 10 个 team\_\* 工具的描述原文（见 team-prompts.md §5.2）
