...
use chisl_api_types::{BedrockConfig, FetchModelsAnonymousRequest, FetchModelsRequest, FetchModelsResponse};
use chisl_common::{AppError, decrypt_string};
use chisl_db::IProviderRepository;
...
mod tests {
    use super::*;
    use chisl_common::encrypt_string;
    use chisl_db::{CreateProviderParams, SqliteProviderRepository, init_database_memory};

    const TEST_KEY: [u8; 32] = [0x42; 32];

    async fn setup() -> (ModelFetchService, chisl_db::Database) {
    ...
    async fn create_provider(db: &chisl_db::Database, platform: &str, base_url: &str, api_key: &str) -> String {
