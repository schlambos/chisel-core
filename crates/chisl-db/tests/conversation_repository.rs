...
async fn setup() -> (SqliteConversationRepository, chisl_db::Database) {
...
fn make_conversation(suffix: &str) -> ConversationRow {
    ...
    ConversationRow {
        id: chisl_common::generate_prefixed_id("conv"),
...
fn make_message(conv_id: &str, content: &str) -> MessageRow {
    let now = chisl_common::now_ms();
    MessageRow {
        id: chisl_common::generate_prefixed_id("msg"),
        conversation_id: conv_id.to_string(),
        msg_id: Some(chisl_common::generate_prefixed_id("cmsg")),
...
fn make_artifact(conv_id: &str, artifact_id: &str) -> chisl_db::ConversationArtifactRow {
    chisl_db::ConversationArtifactRow {
...
async fn pin_and_unpin_conversation() {
    ...
    // Pin
    let pin_time = chisl_common::now_ms();
    repo.update(
        &conv.id,
        &ConversationRowUpdate {
            pinned: Some(true),
...
async fn update_nonexistent_conversation_returns_not_found() {
    ...
    let err = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, chisl_db::DbError::NotFound(_)));
}
