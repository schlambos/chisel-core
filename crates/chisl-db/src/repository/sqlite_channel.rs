...
impl IChannelRepository for SqliteChannelRepository {
    ...
    async fn update_user_last_active(&self, id: &str, last_active: chisl_common::TimestampMs) -> Result<(), DbError> {
    ...
    async fn update_session_activity(
        &self,
        id: &str,
        last_activity: chisl_common::TimestampMs,
    ) -> Result<(), DbError> {
    ...
    async fn cleanup_expired_pairings(&self, now: chisl_common::TimestampMs) -> Result<u64, DbError> {
        let result = sqlx::query(
...
mod tests {
    ...
    async fn upsert_plugin_updates_existing() {
        ...
        let updated = ChannelPluginRow {
            ...
            enabled: true,
            updated_at: chisl_common::now_ms(),
            ..plugin
        };
    ...
    async fn update_user_last_active_updates_timestamp() {
        ...
        let new_ts = chisl_common::now_ms() + 5000;
