...
async fn unauthorized_user_gets_pairing_response() {
    ...
    let repo: Arc<dyn chisl_db::IChannelRepository> = Arc::new(chisl_db::SqliteChannelRepository::new(pool.clone()));
    let bus = Arc::new(chisl_realtime::BroadcastEventBus::new(64));

    let pref_repo: Arc<dyn chisl_db::IClientPreferenceRepository> =
        Arc::new(chisl_db::SqliteClientPreferenceRepository::new(pool));
