...
use chisl_common::{generate_id, now_ms};
use chisl_db::models::AssistantUserRow;
use chisl_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
use chisl_realtime::EventBroadcaster;

use chisl_channel::action::{ActionExecutor, MessageResult};
use chisl_channel::channel_settings::ChannelSettingsService;
use chisl_channel::pairing::PairingService;
use chisl_channel::session::SessionManager;
use chisl_channel::types::{
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn chisl_db::IClientPreferenceRepository> =
        Arc::new(chisl_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
