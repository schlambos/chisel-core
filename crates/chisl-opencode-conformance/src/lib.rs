...
//! `crates/chisl-ai-agent/src/manager/remote/PROTOCOL.md` enumerates every
...
// ---------------------------------------------------------------------------
// SDK version pin (single source of truth).
//
// `version.rs` is generated by `build.rs` from
// `<chisl-root>/opencode-sdk-version.json`. The JSON is the contract; this
// constant is the Rust-side mirror. The AionUi TS sync script reads the
// same JSON, so the two surfaces can never drift silently.
//
// PROTOCOL.md references these constants directly — when bumping the SDK,
// update the JSON, re-run `cargo build -p chisl-opencode-conformance`, and
// re-run `bun run types:sync-opencode` in AionUi. The CI conformance
// workflow (`.github/workflows/conformance.yml`) will fail if the JSON is
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // SDK version-pin contract.
    //
    // These tests guard the single source of truth: the
    // `<chisl-root>/opencode-sdk-version.json` file is read by build.rs and
    // surfaced here as `OPENCODE_SDK_VERSION` (and friends). The AionUi
    // sync script reads the same file; if either side ever stops reading
    // it, the constant in PROTOCOL.md would silently desynchronise from
    // what `package.json` and the live server actually emit.
    // -----------------------------------------------------------------------
    #[test]
    ...
    #[test]
    fn sdk_version_pin_matches_package_name() {
        assert_eq!(OPENCODE_SDK_PACKAGE, "@opencode-ai/sdk");
    }

    #[test]
    fn sdk_version_pin_matches_package_json_dev_dependency() {
        // The Rust side and the AionUi TS side must agree on the pinned
        // version. We read AionUi/package.json from the chisl-root layout
        // so a bumped SDK that only updated one of the two sides fails CI
        // instead of silently drifting.
        let manifest_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        // crates/chisl-opencode-conformance -> ../../.. -> AionCore -> chisl-root.
        let chisl_root = manifest_dir
            .ancestors()
            .nth(3)
            .expect("manifest lives 3 levels below chisl-root");
        ...
        let dev = parsed
            ...
            .unwrap_or_else(|| {
                panic!(
                    "{} devDependencies.{} missing — PROTOCOL.md drift?",
                    package_json_path.display(),
                    OPENCODE_SDK_PACKAGE
                )
            });
        assert_eq!(
            dev, OPENCODE_SDK_VERSION,
            "AionUi/package.json devDependencies.{} = {dev:?} but opencode-sdk-version.json pins {OPENCODE_SDK_VERSION:?}. \
             Bump them together.",
            OPENCODE_SDK_PACKAGE,
        );
    }
