...
use chisl_api_types::WebSocketMessage;
use chisl_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};

use crate::service::ConversationService;
use chisl_db::IConversationRepository;
...
impl StreamRelay {
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let outcome = self
                        .finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(chisl_ai_agent::protocol::events::FinishEventData::default()),
                            current_model.as_ref(),
                        )
                        .await;
                    if self.complete_turn {
    ...
    async fn flush_text_segment(&self, segment: &mut TextSegmentState, model: Option<&AssistantModelInfoEventData>) {
        ...
        if segment.record_created {
            let update = chisl_db::MessageRowUpdate {
            ...
            if let Some(primary_segment) = text_segments.first() {
                if processed.message != text || hidden {
                    ...
                    for segment in text_segments.iter().skip(1) {
                        let hide_update = chisl_db::MessageRowUpdate {
                            content: None,
                            status: Some(Some(status.to_owned())),
                            hidden: Some(true),
                        };
                    ...
                    for segment in text_segments {
                        let status_update = chisl_db::MessageRowUpdate {
                            content: None,
                            status: Some(Some(status.to_owned())),
                            hidden: Some(false),
                        };
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_tool_call(&self, data: &chisl_ai_agent::protocol::events::tool_call::ToolCallEventData) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_acp_tool_call(&self, data: &chisl_ai_agent::protocol::events::tool_call::AcpToolCallEventData) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_tool_group(&self, entries: &[chisl_ai_agent::protocol::events::tool_call::ToolGroupEntry]) {
...
mod tests {
    use super::*;
    use chisl_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    use chisl_db::DbError;
    use std::sync::Mutex;
    ...
    async fn run_text_then_finish_persists_message() {
        ...
        let bus = Arc::new(chisl_realtime::BroadcastEventBus::new(64));
    ...
    async fn run_text_tool_text_splits_text_segments() {
        use chisl_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use chisl_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use chisl_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, chisl_db::MessageRowUpdate)>>,
    }
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, chisl_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _id: &str) -> Result<Option<chisl_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn create(&self, _row: &chisl_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(&self, _id: &str, _updates: &chisl_db::ConversationRowUpdate) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &chisl_db::ConversationFilters,
        ) -> Result<chisl_common::PaginatedResult<chisl_db::models::ConversationRow>, DbError> {
            Ok(chisl_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<chisl_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<chisl_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn get_messages(
            ...
            _page: u32,
            _page_size: u32,
            _order: chisl_db::SortOrder,
        ) -> Result<chisl_common::PaginatedResult<MessageRow>, DbError> {
            Ok(chisl_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
        ...
        async fn update_message(&self, id: &str, updates: &chisl_db::MessageRowUpdate) -> Result<(), DbError> {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<chisl_common::PaginatedResult<chisl_db::MessageSearchRow>, DbError> {
            Ok(chisl_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
