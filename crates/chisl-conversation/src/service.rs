...
use chisl_ai_agent::{AgentInstance, IWorkerTaskManager};
...
pub struct ConversationService {
    ...
    // struct's surface — mirrors the `chisl-ai-agent` `SnapshotDepsRegistry`
...
impl ConversationService {
    ...
    /// `delete()`. Used by `chisl-app` to wire up `WorkerTaskManagerImpl`
    ...
    /// Used by `chisl-app` to wire up remote-session propagation (M06): a
    ...
    pub(crate) fn broadcaster(&self) -> &Arc<dyn chisl_realtime::EventBroadcaster> {
        &self.broadcaster
    }
...
impl ConversationService {
    ...
    async fn create_inner(
        ...
        let row = chisl_db::models::ConversationRow {
            id: id.clone(),
...
impl ConversationService {
            ...
            .ok_or_else(|| {
                AppError::Internal(
                    "Snapshot service not configured; cannot revert tool call. \
                     ...
                     chisl_conversation::snapshot_deps_set(...) (or the per-service \
    ...
    pub async fn tool_call_restore_plan(
        ...
        let actionable = chisl_file::restore_plan_is_actionable(&raw_plan);
        let paths: Vec<RestorePathEntryResponse> = raw_plan
            ...
            .map(|p| RestorePathEntryResponse {
                ...
                operation: match p.operation {
                    chisl_file::RestorePathOperation::Create => RestorePathOperation::Create,
                    chisl_file::RestorePathOperation::Modify => RestorePathOperation::Modify,
                    chisl_file::RestorePathOperation::Delete => RestorePathOperation::Delete,
                    chisl_file::RestorePathOperation::Unknown => RestorePathOperation::Unknown,
                },
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        let opencode_message_id: Option<String> = if row.r#type == "remote" {
            Some(chisl_common::generate_prefixed_id("msg"))
        } else {
            None
        };
        ...
        let user_msg = chisl_db::models::MessageRow {
...
impl ConversationService {
    ...
    fn build_task_options(&self, row: &chisl_db::models::ConversationRow) -> Result<BuildTaskOptions, AppError> {
...
mod tests {
    ...
    fn response_with_type(agent_type: chisl_common::AgentType) -> ConversationResponse {
...
impl ConversationService {
    ...
    pub async fn list_edit_inverses(
        ...
        user_id: &str,
        conversation_id: &str,
    ) -> Result<Vec<chisl_db::EditInverseRow>, AppError> {
    ...
    pub async fn revert_hunk(
        ...
        let total_hunks = chisl_ai_agent::manager::remote::diff_invert::parse_hunks(&row.patch)?.len();
        ...
        let new_content = chisl_ai_agent::manager::remote::diff_invert::revert_single_hunk(
            &row.patch,
            hunk_index,
            &content,
            &row.file_path,
