...
pub use chisl_extension::ResolvedAgentSkill;
...
/// Production adapter backed by `chisl_extension::skill_service`.
pub struct ExtensionSkillResolver {
    paths: Arc<chisl_extension::SkillPaths>,
}

impl ExtensionSkillResolver {
    pub fn new(paths: Arc<chisl_extension::SkillPaths>) -> Self {
        Self { paths }
    }
}
...
impl SkillResolver for ExtensionSkillResolver {
    async fn auto_inject_names(&self) -> Vec<String> {
        match chisl_extension::list_builtin_auto_skills(&self.paths).await {
            Ok(items) => {
    ...
    async fn resolve_skills(&self, names: &[String]) -> Vec<ResolvedAgentSkill> {
        ...
        match chisl_extension::materialize_skills_for_agent(&self.paths, "workspace-link", names).await {
            Ok(list) => list,
            Err(e) => {
                tracing::warn!(
                    error = %e,
    ...
    async fn link_workspace_skills(&self, workspace: &Path, rel_dirs: &[&str], skills: &[ResolvedAgentSkill]) -> usize {
        ...
        match chisl_extension::link_workspace_skills(workspace, rel_dirs, skills).await {
            Ok(n) => n,
            Err(e) => {
                tracing::warn!(
