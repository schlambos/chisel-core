...
use chisl_ai_agent::AgentInstance;
use chisl_ai_agent::protocol::events::{AgentStreamEvent, OpencodeSessionCompactedData};
use chisl_api_types::{
...
impl ConversationService {
    ...
    pub async fn delete_remote_message(&self, conversation_id: &str, row_id: &str) -> Result<(), AppError> {
        ...
        self.broadcaster().broadcast(chisl_api_types::WebSocketMessage::new(
            "message.removed",
    ...
    fn broadcast_opencode_session_compacted(&self, conversation_id: &str, data: &OpencodeSessionCompactedData) {
        ...
        chisl_common::normalize_keys_to_snake_case(&mut event_data);
        let payload = json!({
            "conversation_id": conversation_id,
