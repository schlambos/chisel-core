...
use chisl_ai_agent::protocol::events::{AgentStreamEvent, FinishEventData, TextEventData};
...
use chisl_api_types::ConversationArtifactKind;
use chisl_api_types::{
...
impl IConversationRepository for MockRepo {
    async fn get(&self, id: &str) -> Result<Option<ConversationRow>, chisl_db::DbError> {
        let rows = self.rows.lock().unwrap();
        Ok(rows.iter().find(|r| r.id == id).cloned())
    }

    async fn create(&self, row: &ConversationRow) -> Result<(), chisl_db::DbError> {
    ...
    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), chisl_db::DbError> {
        ...
        let row = rows
            .iter_mut()
            .find(|r| r.id == id)
            .ok_or_else(|| chisl_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, id: &str) -> Result<(), chisl_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let len_before = rows.len();
        rows.retain(|r| r.id != id);
        if rows.len() == len_before {
            return Err(chisl_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, chisl_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, chisl_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, chisl_db::DbError> {
        Ok(vec![])
    }
    ...
    async fn get_messages(
        ...
        page_size: u32,
        order: SortOrder,
    ) -> Result<PaginatedResult<MessageRow>, chisl_db::DbError> {
    ...
    async fn insert_message(&self, message: &MessageRow) -> Result<(), chisl_db::DbError> {
        self.messages.lock().unwrap().push(message.clone());
        Ok(())
    }

    async fn update_message(&self, id: &str, updates: &MessageRowUpdate) -> Result<(), chisl_db::DbError> {
        let mut messages = self.messages.lock().unwrap();
        let message = messages
            .iter_mut()
            .find(|message| message.id == id)
            .ok_or_else(|| chisl_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, conv_id: &str) -> Result<(), chisl_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, chisl_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_artifacts(&self, conversation_id: &str) -> Result<Vec<ConversationArtifactRow>, chisl_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, chisl_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn mark_skill_suggest_artifacts_saved(
        ...
        cron_job_id: &str,
        updated_at: TimestampMs,
    ) -> Result<Vec<ConversationArtifactRow>, chisl_db::DbError> {
        let mut artifacts = self.artifacts.lock().unwrap();
    ...
    async fn delete_artifacts_by_conversation(&self, conversation_id: &str) -> Result<(), chisl_db::DbError> {
        self.artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, chisl_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
async fn update_invokes_registered_hook() {
    use chisl_common::OnConversationUpdate;
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AppError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            chisl_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = chisl_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
