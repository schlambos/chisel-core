...
use chisl_common::{AgentKillReason, AppError, TimestampMs};
use chisl_conversation::ConversationService;
use chisl_conversation::skill_resolver::SkillResolver;
use chisl_db::IEditInverseRepository;
use chisl_db::IOpencodeToolSnapshotRepository;
use chisl_db::models::{EditInverseRow, OpencodeToolSnapshotRow};
use chisl_db::{SqliteConversationRepository, SqliteEditInverseRepository, init_database_memory};
use chisl_file::{ISnapshotService, SnapshotService};
...
struct TestBroadcaster {
    events: Mutex<Vec<chisl_api_types::WebSocketMessage<serde_json::Value>>>,
}
...
impl EventBroadcaster for TestBroadcaster {
    fn broadcast(&self, event: chisl_api_types::WebSocketMessage<serde_json::Value>) {
...
impl IWorkerTaskManager for NoopTaskManager {
    fn get_task(&self, _: &str) -> Option<chisl_ai_agent::AgentInstance> {
        None
    }
    async fn get_or_build_task(
        &self,
        _: &str,
        _: chisl_ai_agent::types::BuildTaskOptions,
...
impl SkillResolver for EmptySkillResolver {
    ...
    async fn resolve_skills(&self, _names: &[String]) -> Vec<chisl_extension::ResolvedAgentSkill> {
        Vec::new()
    }
    async fn link_workspace_skills(
        ...
        _rel_dirs: &[&str],
        _skills: &[chisl_extension::ResolvedAgentSkill],
    ) -> usize {
        0
    }
}

async fn setup() -> (ConversationService, chisl_db::SqlitePool, TempDir) {
    let db = init_database_memory().await.unwrap();
    let pool = db.pool().clone();
    let conversation_repo: Arc<dyn chisl_db::IConversationRepository> =
        Arc::new(SqliteConversationRepository::new(pool.clone()));
    let agent_metadata_repo: Arc<dyn chisl_db::IAgentMetadataRepository> =
        Arc::new(chisl_db::SqliteAgentMetadataRepository::new(pool.clone()));
    let acp_session_repo: Arc<dyn chisl_db::IAcpSessionRepository> =
        Arc::new(chisl_db::SqliteAcpSessionRepository::new(pool.clone()));
...
fn wire_edit_inverse_repo(svc: &ConversationService, pool: &chisl_db::SqlitePool) -> Arc<dyn IEditInverseRepository> {
...
fn wire_snapshot_deps(
    ...
    let tool_snapshot_repo: Arc<dyn IOpencodeToolSnapshotRepository> =
        Arc::new(chisl_db::SqliteOpencodeToolSnapshotRepository::new(pool.clone()));
...
async fn seed_conversation(svc: &ConversationService) -> String {
    let req: chisl_api_types::CreateConversationRequest = serde_json::from_value(json!({
        "type": "acp",
