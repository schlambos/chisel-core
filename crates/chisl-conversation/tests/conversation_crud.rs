...
use chisl_common::{AgentKillReason, AgentType, AppError, ConversationSource, ConversationStatus, TimestampMs};
...
async fn setup() -> (ConversationService, Arc<TestBroadcaster>, Arc<dyn IWorkerTaskManager>) {
    ...
    let agent_metadata_repo: Arc<dyn chisl_db::IAgentMetadataRepository> =
        Arc::new(chisl_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn chisl_db::IAcpSessionRepository> =
        Arc::new(chisl_db::SqliteAcpSessionRepository::new(db.pool().clone()));
...
async fn t3_2_get_not_found() {
    ...
    assert!(matches!(err, chisl_common::AppError::NotFound(_)));
}
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use chisl_db::{SqliteAcpSessionRepository, init_database_memory};

    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(chisl_db::SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let agent_metadata_repo: Arc<dyn chisl_db::IAgentMetadataRepository> =
        Arc::new(chisl_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn chisl_db::IAcpSessionRepository> =
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = chisl_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
