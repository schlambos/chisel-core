...
use chisl_ai_agent::manager::aionrs::AionrsAgentManager;
use chisl_ai_agent::task_manager::AgentFactory;
use chisl_ai_agent::types::{AionrsResolvedConfig, BuildTaskOptions, SendMessageData};
use chisl_ai_agent::*;
use chisl_ai_agent::{SkillIndex, build_system_instructions_with_skills_index};
use chisl_common::{AgentKillReason, AgentType, ConversationStatus, ProviderWithModel, TimestampMs, now_ms};
use serde_json::json;
...
impl IAgentTask for TypedMockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), chisl_common::AppError> {
        Ok(())
    }
