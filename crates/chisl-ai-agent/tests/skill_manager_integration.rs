...
use chisl_extension::{BUILTIN_SKILLS_ENV_VAR, resolve_skill_paths};
...
fn system_instructions_with_loaded_skills() {
    let skills = vec![chisl_ai_agent::SkillDefinition {
        name: "helper".into(),
        description: "A helper".into(),
        location: std::path::PathBuf::new(),
        source: chisl_extension::SkillSource::Custom,
...
fn first_message_with_skills_index_for_acp() {
    let skills = vec![chisl_ai_agent::SkillIndex {
        name: "review".into(),
