...
use chisl_ai_agent::AcpSessionSyncService;
use chisl_ai_agent::AcpSkillManager;
use chisl_ai_agent::factory::{AgentFactoryDeps, build_agent_factory};
use chisl_ai_agent::registry::AgentRegistry;
use chisl_ai_agent::types::BuildTaskOptions;
use chisl_common::{AgentType, ProviderWithModel, encrypt_string};
use chisl_db::{
...
fn make_factory(
...
) -> chisl_ai_agent::task_manager::AgentFactory {
    let tmp = tempfile::TempDir::new().unwrap();
    let skill_paths = Arc::new(chisl_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    build_agent_factory(AgentFactoryDeps {
        ...
        work_dir: PathBuf::from("/tmp/aionrs-test"),
        backend_binary_path: Arc::new(PathBuf::from("/tmp/aionrs-test/chislcore")),
