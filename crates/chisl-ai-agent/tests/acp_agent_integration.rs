...
use chisl_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use chisl_ai_agent::manager::acp::AcpAgentManager;
use chisl_ai_agent::registry::AgentRegistry;
use chisl_ai_agent::{AgentInstance, AgentStreamEvent, IAgentTask};
use chisl_common::ConversationStatus;
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = chisl_ai_agent::AcpBuildExtra {
        agent_id: None,
    ...
    let skill_paths = std::sync::Arc::new(chisl_extension::resolve_skill_paths(
        tmp_skills.path(),
        tmp_skills.path(),
    ));
    let skill_manager = chisl_ai_agent::AcpSkillManager::new(skill_paths);
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: chisl_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), chisl_common::AgentType::Acp);
...
async fn acp_agent_kill_terminates_process() {
    ...
    agent.kill(Some(chisl_common::AgentKillReason::IdleTimeout)).unwrap();
