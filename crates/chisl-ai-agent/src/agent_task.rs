...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<chisl_api_types::AgentModeResponse, AppError> {
        Ok(chisl_api_types::AgentModeResponse {
            mode: "default".into(),
            initialized: false,
...
impl AgentInstance {
    ...
    pub fn get_confirmations(&self) -> Vec<chisl_common::Confirmation> {
        match self {
            Self::Acp(_) => Vec::new(),
    ...
    pub async fn get_mode(&self) -> Result<chisl_api_types::AgentModeResponse, AppError> {
        match self {
            Self::Acp(m) => m.mode().await,
            Self::Aionrs(m) => m.mode().await,
            Self::Remote(m) => m.mode().await,
            Self::OpenClaw(_) | Self::Nanobot(_) => Ok(chisl_api_types::AgentModeResponse {
                mode: "default".into(),
                initialized: false,
                available_modes: None,
            }),
    ...
    /// [`chisl_common::normalize_keys_to_snake_case`] so keys land as
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AppError> {
        match self {
            Self::Acp(m) => {
                ...
                chisl_common::normalize_keys_to_snake_case(&mut value);
                Ok(Some(value))
            }
