...
async fn backfill_remote_history(
    ...
    let existing = state
        .conversation_repo
        .get_messages(&conversation_id, 0, 1, chisl_db::SortOrder::Asc)
        .await
...
async fn mark_loaded_and_respond(
    ...
    let updates = chisl_db::ConversationRowUpdate {
        name: None,
        pinned: None,
        pinned_at: None,
        model: None,
        ...
        status: None,
        updated_at: Some(chisl_common::now_ms()),
    };
    state
        .conversation_repo
...
async fn list_bg_processes(
    ...
    Path(id): Path<String>,
    Extension(_user): Extension<CurrentUser>,
) -> Result<Json<ApiResponse<chisl_api_types::BgProcessListResponse>>, AppError> {
    ...
    Ok(Json(ApiResponse::ok(chisl_api_types::BgProcessListResponse {
        processes,
    })))
}
...
async fn stop_bg_process(
    ...
    Extension(_user): Extension<CurrentUser>,
) -> Result<Json<ApiResponse<chisl_api_types::BgProcessUiInfo>>, AppError> {
    state.service.get(&id).await?;
...
async fn read_bg_process_output(
...
) -> Result<Json<ApiResponse<chisl_api_types::BgProcessOutputResponse>>, AppError> {
    state.service.get(&id).await?;
    ...
    Ok(Json(ApiResponse::ok(chisl_api_types::BgProcessOutputResponse {
        output,
        next_offset,
        process,
    })))
