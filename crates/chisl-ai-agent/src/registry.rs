...
use chisl_api_types::{AgentEnvEntry, AgentHandshake, AgentMetadata, AgentSource, AgentSourceInfo, BehaviorPolicy};
use chisl_common::{AgentType, AppError};
use chisl_db::{AgentMetadataRow, IAgentMetadataRepository, UpdateAgentHandshakeParams};
use chisl_runtime::resolve_command_path;
...
fn decode_row(row: AgentMetadataRow) -> Option<(AgentMetadata, Option<UnavailableReason>)> {
    ...
    let team_capable = behavior_policy.supports_team
        || chisl_common::constants::is_team_capable(backend_str, handshake.agent_capabilities.as_ref());
...
/// unavailable rows go to `info!` so the default chislcore.log surfaces
