...
impl PermissionRouter {
    ...
    pub fn confirm(
        ...
        let responder = self
            ...
            .remove(call_id)
            .ok_or_else(|| {
                chisl_common::AppError::BadRequest(format!("Pending ACP permission not found: {call_id}"))
            })?;

        responder
            .send(PermissionDecision::Selected { option_id })
            .map_err(|_| chisl_common::AppError::BadRequest(format!("Pending ACP permission expired: {call_id}")))?;
