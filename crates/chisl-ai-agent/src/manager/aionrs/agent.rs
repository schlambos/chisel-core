...
impl AionrsAgentManager {
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<chisl_api_types::SlashCommandItem>, AppError> {
        let engine = self.engine.lock().await;
        Ok(engine
            .slash_command_list()
            .into_iter()
            .map(|(command, description)| chisl_api_types::SlashCommandItem { command, description })
            .collect())
    }
}
