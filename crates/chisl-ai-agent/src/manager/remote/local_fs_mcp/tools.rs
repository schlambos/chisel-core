...
mod tests {
    ...
    use chisl_db::IUserRepository;
    use chisl_db::SqliteOpencodeToolSnapshotRepository;
    ...
    struct HookHarness {
        #[allow(dead_code)]
        snapshot_service: Arc<SnapshotService>,
        tool_snapshot_repo: Arc<dyn chisl_db::IOpencodeToolSnapshotRepository>,
        conversation_id: String,
        workspace_root: PathBuf,
        hook: SnapshotHook,
    }
    ...
    async fn make_hook_harness() -> (HookHarness, TempDir) {
        ...
        let user_repo = chisl_db::SqliteUserRepository::new(pool.clone());
        let user = user_repo.create_user("tester", "").await.expect("seed user");
        let conversation_repo = chisl_db::SqliteConversationRepository::new(pool.clone());
        ...
        let tool_snapshot_repo: Arc<dyn chisl_db::IOpencodeToolSnapshotRepository> =
