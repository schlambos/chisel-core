...
//! manager ([`super::bg`]) live inside `chisl-ai-agent` and have no
//! direct handle to the host's [`chisl_realtime::EventBroadcaster`]
//! (the broadcaster is constructed in `chisl-app` and lives behind an
...
//! the plugin pipeline importing `chisl-realtime` directly — a
//! dependency arrow that would create a cycle
//! (`chisl-realtime` already depends on `chisl-api-types`, and
//! `chisl-app` composes both).
//!
