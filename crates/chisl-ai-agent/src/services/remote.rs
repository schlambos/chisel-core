...
use chisl_db::models::RemoteAgentRow;
use chisl_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
use base64::Engine;
...
impl RemoteAgentService {
    ...
    pub async fn create(&self, req: CreateRemoteAgentRequest) -> Result<RemoteAgentResponse, AppError> {
        ...
        let row = self
            .repo
            .create(chisl_db::CreateRemoteAgentParams {
                name: &req.name,
    ...
    pub async fn update(&self, id: &str, req: UpdateRemoteAgentRequest) -> Result<RemoteAgentResponse, AppError> {
        ...
        let row = self.repo.update(id, params).await.map_err(|e| match e {
            chisl_db::DbError::NotFound(msg) => AppError::NotFound(msg),
    ...
    pub async fn fetch_agents(&self, id: &str) -> Result<Vec<chisl_api_types::AgentModeOption>, AppError> {
        let row = self
            .repo
            .find_by_id(id)
            .await
    ...
    pub async fn fetch_session_messages(
        ...
        conversation_id: &str,
        session_id: &str,
    ) -> Result<Vec<chisl_db::models::MessageRow>, AppError> {
        let row = self
            .repo
    ...
    pub async fn ping_health(&self, id: &str) -> Result<chisl_api_types::RemoteAgentHealthResponse, AppError> {
        let row = self
            .repo
            .find_by_id(id)
            .await
        ...
        match protocol {
            RemoteAgentProtocol::OpenCode => {
                ...
                match result {
                    Ok(()) => Ok(chisl_api_types::RemoteAgentHealthResponse {
                        healthy: true,
                        latency_ms,
                        error: None,
                    }),
                    Err(e) => Ok(chisl_api_types::RemoteAgentHealthResponse {
                        healthy: false,
                        latency_ms,
            ...
            RemoteAgentProtocol::ZeroClaw => Ok(chisl_api_types::RemoteAgentHealthResponse {
                healthy: false,
                latency_ms: 0,
    ...
    pub async fn update_session(
        ...
        let Some(body) = build_session_patch_body(&patch, chisl_common::now_ms()) else {
            return Ok(());
        };
...
async fn fetch_opencode_agents(
    ...
    auth_token: Option<&str>,
    allow_insecure: bool,
) -> Result<Vec<chisl_api_types::AgentModeOption>, AppError> {
...
pub(crate) fn convert_opencode_messages(
...
) -> Vec<chisl_db::models::MessageRow> {
    ...
    for msg in array {
        ...
        let base_created = info
            ...
            .and_then(|v| v.as_f64())
            .map(normalize_ms)
            .unwrap_or_else(chisl_common::now_ms);
...
fn build_retry_row(
    ...
    message_id: &str,
    part_id: &str,
) -> Option<chisl_db::models::MessageRow> {
    ...
    Some(chisl_db::models::MessageRow {
...
fn part_id_str_from_part() -> String {
    chisl_common::generate_short_id()
}
...
fn generate_device_keypair(encryption_key: &[u8; 32]) -> Result<(String, String, String), AppError> {
    ...
    let device_id = chisl_common::generate_prefixed_id("dev");
...
fn db_err(e: chisl_db::DbError) -> AppError {
...
mod tests {
    ...
    async fn build_service_with_agent(name: &str) -> (RemoteAgentService, chisl_db::Database) {
        use chisl_db::CreateRemoteAgentParams;
        use chisl_db::SqliteRemoteAgentRepository;
        use chisl_db::init_database_memory;
        let db = init_database_memory().await.unwrap();
        let pool = db.pool().clone();
        let repo: Arc<dyn chisl_db::IRemoteAgentRepository> = Arc::new(SqliteRemoteAgentRepository::new(pool));
