...
use chisl_api_types::{AionrsBuildExtra, GuideMcpConfig, TEAM_MCP_SERVER_NAME, TeamMcpStdioConfig};
...
pub(super) async fn build(
    ...
    let api_key = chisl_common::decrypt_string(&row.api_key_encrypted, &deps.encryption_key)?;
...
fn resolve_bedrock_config(json: Option<&str>) -> Option<aion_config::config::BedrockConfig> {
    let bc: chisl_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
