use chisl_common::{AppError, CommandSpec, ErrorChain};
use chisl_runtime::Builder as CmdBuilder;
use std::path::Path;
use std::sync::Arc;
...
impl CliAgentProcess {
    ...
    fn agent_spawn_env(data_dir: &Path) -> Vec<(String, String)> {
        ...
        // `chisl_runtime::enhance_process_path` during startup; children
    ...
    /// to `chisl_runtime::resolve_command_in`, which honours `PATHEXT` on
    ...
    fn find_native_claude() -> Option<String> {
        ...
        for dir in std::env::split_paths(&path_var) {
            ...
            if let Some(found) = chisl_runtime::resolve_command_in("claude", &dir) {
