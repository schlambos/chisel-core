...
pub struct SkillDefinition {
    ...
    pub source: chisl_extension::SkillSource,
...
pub struct AcpSkillManager {
    ...
    #[allow(dead_code)]
    paths: Arc<chisl_extension::SkillPaths>,
}

impl AcpSkillManager {
    pub fn new(paths: Arc<chisl_extension::SkillPaths>) -> Arc<Self> {
        Arc::new(Self {
    ...
    /// Discover skills via `chisl_extension::list_available_skills`.
    ///
    /// Filtering rules:
    ...
    pub async fn discover_skills(
        ...
        let items = match chisl_extension::list_available_skills(&self.paths).await {
            Ok(v) => v,
            Err(e) => {
        ...
        for item in items {
            ...
            let keep = match item.source {
                chisl_extension::SkillSource::Builtin => {
                    if is_auto_inject {
                ...
                chisl_extension::SkillSource::Custom | chisl_extension::SkillSource::Extension => {
                    enabled_skills.is_some_and(|en| en.iter().any(|n| n == &item.name))
                }
            };
            if !keep {
    ...
    /// - `Builtin` → `chisl_extension::read_builtin_skill(&paths, relative)`
        ...
        let content = match def.source {
            chisl_extension::SkillSource::Builtin => {
                if let Some(rel) = def.relative_location.as_deref() {
                    match chisl_extension::read_builtin_skill(&self.paths, rel).await {
                        Ok(s) => s,
                        Err(e) => {
...
mod tests {
    ...
    async fn new_accepts_skill_paths() {
        ...
        let paths = std::sync::Arc::new(chisl_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    ...
    fn skill_definition_has_source_and_relative_location() {
        let def = SkillDefinition {
            ...
            source: chisl_extension::SkillSource::Builtin,
            relative_location: Some("auto-inject/x/SKILL.md".into()),
            body: None,
        };
        assert_eq!(def.source, chisl_extension::SkillSource::Builtin);
    ...
    // because they now need `chisl_extension::BUILTIN_SKILLS_ENV_VAR` to
    ...
    async fn get_skill_unknown_returns_none() {
        let tmp = TempDir::new().unwrap();
        let mgr = AcpSkillManager::new(std::sync::Arc::new(chisl_extension::resolve_skill_paths(
            tmp.path(),
            tmp.path(),
        )));
