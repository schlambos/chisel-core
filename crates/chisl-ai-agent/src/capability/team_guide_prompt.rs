...
//!
//! This is a local copy of the prompt owned by `chisl_team::prompts::team_guide`
//! (see `crates/aionui-team/src/prompts/team_guide.rs`). We duplicate rather
//! than import because `aionui-team` depends on `chisl-ai-agent`, so the
//! reverse direction would create a dependency cycle. Future work may sink
//! this prompt into `chisl-common` and have both crates re-export; until
...
use chisl_common::constants::TEAM_CAPABLE_BACKENDS;
