...
use chisl_extension::hub::{HubIndexManager, HubInstaller};
use chisl_extension::registry::ExtensionRegistry;
use chisl_extension::state::ExtensionStateStore;
use chisl_extension::types::HubExtensionStatus;
use chisl_realtime::BroadcastEventBus;
use serde_json::json;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![chisl_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: chisl_extension::types::ExtensionSource::Env,
    }];
