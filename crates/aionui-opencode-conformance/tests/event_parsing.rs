...
//!   2. The event passes through [`aionui_opencode_conformance::unwrap_event`]
//!      (the same normalizer the live adapter uses).
//!   3. [`aionui_opencode_conformance::classify_event`] is asserted to return
...
//!
//! See `PROTOCOL.md` (in `crates/aionui-ai-agent/src/manager/remote/`) for the
...
use aionui_opencode_conformance::{ClassifyOutcome, EventKind, classify_event, unwrap_event};
...
fn all_recorded_fixtures_classify_without_panic() -> Result<()> {
    ...
    if tally.unknown.is_empty() {
        ...
        eprintln!("  (add these to crates/aionui-opencode-conformance/TODO.md or promote them)\n");
    }
