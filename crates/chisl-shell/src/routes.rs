...
async fn speech_to_text(
    ...
    let config: SpeechToTextConfig = prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: chisl_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        });
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(chisl_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = chisl_system::ClientPrefService::new(repo);
