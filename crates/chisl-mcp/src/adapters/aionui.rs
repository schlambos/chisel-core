...
mod tests {
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn find_by_id(&self, id: &str) -> Result<Option<McpServerRow>, chisl_db::DbError> {
            Ok(self.servers.iter().find(|s| s.id == id).cloned())
        }

        async fn find_by_name(&self, name: &str) -> Result<Option<McpServerRow>, chisl_db::DbError> {
