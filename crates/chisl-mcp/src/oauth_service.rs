...
use chisl_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use chisl_common::{TimestampMs, now_ms};
use chisl_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
impl McpOAuthService {
    ...
    pub async fn logout(&self, server_url: &str) -> Result<(), McpError> {
        match self.token_repo.delete(server_url).await {
            ...
            Err(chisl_db::DbError::NotFound(_)) => {
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<chisl_db::models::OAuthTokenRow>, chisl_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<chisl_db::models::OAuthTokenRow, chisl_db::DbError> {
            unimplemented!()
        }

        async fn delete(&self, _: &str) -> Result<(), chisl_db::DbError> {
            Ok(())
        }

        async fn list_authenticated_urls(&self) -> Result<Vec<String>, chisl_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, url: &str) -> Result<(), chisl_db::DbError> {
            Err(chisl_db::DbError::NotFound(format!(
    ...
    impl IOAuthTokenRepository for ValidTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<chisl_db::models::OAuthTokenRow>, chisl_db::DbError> {
            Ok(Some(chisl_db::models::OAuthTokenRow {
