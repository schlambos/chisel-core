...
use chisl_api_types::{CreateMcpServerRequest, McpTransport};
use chisl_common::McpSource;
use chisl_db::SqliteMcpServerRepository;
use chisl_mcp::{DetectedServer, McpAgentAdapter, McpConfigService, McpError, McpServerTransport, McpSyncService};
...
async fn make_services(adapters: Vec<Arc<dyn McpAgentAdapter>>) -> (McpConfigService, McpSyncService) {
    ...
    let repo: Arc<dyn chisl_db::IMcpServerRepository> = Arc::new(SqliteMcpServerRepository::new(db.pool().clone()));
